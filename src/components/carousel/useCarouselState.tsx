...
export const useCarouselState = ({
  ...
  const extractKeywordsFromText = (text: string, businessInfo: BusinessInfo): string => {
    ...
    const keyWords = words.filter((word) => word.length > 3 && !stopWords.includes(word)).slice(0, 3) // Reduzir para 3 palavras-chave principais

    // Extrair substantivos e verbos importantes
    const importantWords = words.filter((word) => {
      return !stopWords.includes(word) && word.length > 3
    })

    // Detectar temas específicos no texto de forma mais precisa
    const themes = []

    // Mapeamento de temas mais específicos
    const themeMapping: Record<string, string[]> = {
      férias: ["vacation", "travel", "holiday"],
      segurança: ["car safety", "vehicle security"],
      família: ["family car", "family travel"],
      revisão: ["car inspection", "vehicle check"],
      manutenção: ["car maintenance", "auto service"],
      óleo: ["oil change", "car service"],
      motor: ["car engine", "engine service"],
      preço: ["fair price", "affordable service"],
      qualidade: ["quality service", "professional service"],
      agendar: ["schedule service", "book appointment"],
    }

    // Verificar palavras-chave no texto e adicionar temas relacionados
    ...
    // Se não encontrou temas específicos, usar contexto do negócio
    if (themes.length === 0) {
      themes.push(businessInfo.industry, "professional service")
    }

    // Adicionar modificadores visuais mais específicos
    const visualModifiers = [
      "professional photo",
      "high quality image",
      "commercial photography",
      "business photo",
      "professional service photo",
    ]

    // Selecionar dois modificadores aleatórios diferentes
    const shuffledModifiers = visualModifiers.sort(() => Math.random() - 0.5)
    const selectedModifiers = shuffledModifiers.slice(0, 2)

    // Combinar todos os elementos de forma mais estruturada
    const searchTerms = [
      ...keyWords, // Palavras-chave do texto
      ...themes.slice(0, 2), // Temas específicos
      ...selectedModifiers, // Modificadores visuais
      businessInfo.industry, // Indústria do negócio
      Math.floor(Math.random() * 1000), // Número aleatório para variedade
    ].filter(Boolean)

    // Juntar os termos e garantir que não há duplicatas
    const uniqueTerms = Array.from(new Set(searchTerms))
    const finalSearchQuery = uniqueTerms.join(" ")

    console.log("Termos de busca gerados:", {
      originalText: text,
      keyWords,
      themes,
  ...
  const handleRegenerateTexts = async () => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Erro ao gerar textos")
    } finally {
      setIsLoading(false)
    }
  }
